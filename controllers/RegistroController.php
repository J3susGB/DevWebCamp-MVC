...
class RegistroController {
    ...
    public static function gratis(Router $router) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $token = substr( md5(uniqid(rand(), true)), 0, 8); //substr cortará el token desde el inicio hasta el caracter 8
            ...
            if( $resultado ) {
                header('Location: /boleto?id=' . urlencode($registro->token)); //urlencode para evitar caracteres especiales
                return;
            }
        }
    }
    ...
    public static function conferencias(Router $router) {
        ...
        if( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
            ...
            //Obetener el registro de usuario:
            $registro = Registro::where('usuario_id', $_SESSION['id']);
            if( !isset($registro) || $registro->paquete_id !== "1" ) {
                echo json_encode(['resultado' => false]);
                return;
